...
def precalculateTpSl(df, tpCriteria, slCriteria):
    ...
    if tpCriteria is TakeProfitAnchor.PREV_CANDLE_OPEN:
        ...
        df['limit'] = np.where(df['isBull'], df['close'] + (df['close'] - df['stop']), df['close'] - (df['stop'] - df['close']))


def precalculateTooSmallTrade(df):

    df['buySignal'] = np.where(df['buySignal'], ~(((df['limit'] - df['close']) / df['close'] * 100 < 0.2) | ((df['close'] - df['stop']) / df['close'] * 100 < 0.2)), df['buySignal'])

    df['sellSignal'] = np.where(df['sellSignal'], ~(((df['stop'] - df['close']) / df['close'] * 100 < 0.2) | ((df['close'] - df['limit']) / df['close'] * 100 < 0.2)), df['sellSignal'])
...
def runTradeStrategy(datafileName,
    ...
    for index, row in df.iterrows():
        ...
        if row['buySignal']:
            ...
            currTrades.append({'openTime': row['close_datetime'], 'side': 'long',
                               'type': lastRow['candleColor'] + row['candleColor'],
                               'limit': row['limit'], 'stop': row['stop'],
                               'WL': "", 'closeTime': '',
                               'rr': row['rr']})

        elif row['sellSignal']:

            currTrades.append({'openTime': row['close_datetime'], 'side': 'short',
                               'type': lastRow['candleColor'] + row['candleColor'],
                               'limit': row['limit'], 'stop': row['stop'],
                               'WL': "", 'closeTime': '',
                               'rr': row['rr']})
